...
export class Plugin extends BSBEvents<Config> {
  ...
  constructor(config: BSBEventsConstructor) {
    super(config);
    this.broadcast = new broadcast(this, this.createNewLogger("broadcast"), this.metrics);
    this.emit = new emit(this, this.createNewLogger("emit"), this.metrics);
    this.ear = new emitAndReturn(this, this.createNewLogger("emitAndReturn"), this.metrics);
  ...
  public getPlatformName(name: string): string {
    if (this.config.platformKey === null) return name;
    return `${ name }-${ this.config.platformKey }`;
  }

  async init(trace: DTrace): Promise<void> {
    await this._connectToAMQP(trace);
  }

  private async _connectToAMQP(trace: DTrace) {
    this.log.info(trace, `Connect to {endpoints}`, {
    ...
    this.publishConnection.on("connect", async (data: any) => {
      self.log.info(trace, "AMQP CONNECTED: {url}", { url: data.url });
    });
    this.publishConnection.on(
      ...
      async (data: any): Promise<any> => {
        if (
          ...
          self.log.error(trace, "AMQP CONNECT FAIL: {url} ({msg})", {
            url: data.url,
    ...
    this.publishConnection.on("error", async (err: any) => {
      if (err.message !== "Connection closing") {
        self.log.error(trace, "AMQP ERROR: {message}", { message: err.message });
      }
      if (self.config.fatalOnDisconnect) {
        self.log.error(trace, "AMQP ERROR: {message}", {
          message: err.message,
        });
        process.exit(5);
      }
    ...
    this.publishConnection.on("close", async (): Promise<any> => {
      self.log.warn(trace, "AMQP CONNECTION CLOSED");
    });
    this.receiveConnection.on("close", async (): Promise<any> => {
      self.log.warn(trace, "AMQP CONNECTION CLOSED");
    });

    this.log.info(trace, `Connected to {endpoints}x2? (s:{sendS}/p:{pubS})`, {
      endpoints: this.config.endpoints,
    ...
    await this.broadcast.init(trace);
    await this.emit.init(trace);
    await this.ear.init(trace);
  }
